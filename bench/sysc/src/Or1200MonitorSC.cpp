...
#define FMTLEN 2000
	...
	i = 0;
	while (memaccessor->get_mem8(fmtaddr) != '\0') {
		fmtstr[i++] = memaccessor->get_mem8(fmtaddr);
		fmtaddr++;
		if (i == FMTLEN - 1)
			break;
	}
		...
		if (fmtstrpart[index] == '\0') {
			...
			if (fmtstrpart[index] == 's') {
				...
				oraddr_t ormem_str_ptr =
				    memaccessor->get_mem32(argaddr);

				while (memaccessor->get_mem8(ormem_str_ptr++) !=
				       '\0')
					tmp_string_len++;
				...
				ormem_str_ptr = memaccessor->get_mem32(argaddr);	/* Reset start pointer value */
				for (z = 0; z < tmp_string_len; z++)
					str[z] =
					    memaccessor->get_mem8(ormem_str_ptr +
							       z);
				...
				/* 
				   ...
				   value
				 */
				arg = memaccessor->get_mem32(argaddr);
