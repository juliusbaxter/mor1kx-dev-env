...
`define sg708 // Pick the RAM model it is for Nexys 3
...
`include "cellram_parameters.vh"
   ...
   parameter preload_file = "flash16.in";
   
   
   task preload_mem;
      reg [15:0] cellram_load_data [0:8192];
      integer    cellram_data_load_ctr;
      reg 	 itexists;
      
      begin
	 $readmemh(preload_file, cellram_load_data);
	 
	 for (cellram_data_load_ctr=0;cellram_data_load_ctr<8192;cellram_data_load_ctr=cellram_data_load_ctr+1)
	   begin
	      
	      if (!(cellram_load_data[cellram_data_load_ctr]===16'hxxxx)  && !memory_addr_exists(cellram_data_load_ctr))
		begin
		   memory_addr[memory_index] = cellram_data_load_ctr;
		   memory[memory_index] = cellram_load_data[cellram_data_load_ctr];
		   memory_used = memory_used + 1;
		   //$display("Preloading RAM at %08h with %04h", cellram_data_load_ctr, cellram_load_data[cellram_data_load_ctr]);
		end
